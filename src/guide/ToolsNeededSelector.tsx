...
const ToolsNeededSelector = ({ stepIndex }: ToolsNeededSelectorProps) => {
  const { control } = useFormContext<GuideForm>();

  return (
    <Controller
      control={control}
      name={`steps.${stepIndex}.toolsNeeded`} // ✅ this is fine
      render={({ field }) => {
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      const updated = checked
                        ? [...value, tool.id]
                        : value.filter((id: string) => id !== tool.id);
...
export default ToolsNeededSelector;
